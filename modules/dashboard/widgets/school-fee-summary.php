...
if (!empty($childProfiles)) {
    foreach ($childProfiles as $child) {
        ...
        $schoolFeeController->calculateFees($child);
        $results = $schoolFeeController->getCalculationResults();
        
        if (!empty($results) && !empty($results['yearlyProjections'])) {
            // Find current year projection
            foreach ($results['yearlyProjections'] as $projection) {
                if ($projection['year'] == $currentYear) {
                    $totalUpcomingExpenses += $projection['total'];
                    $childrenWithUpcomingExpenses[] = [
                        'name' => $child['name'],
