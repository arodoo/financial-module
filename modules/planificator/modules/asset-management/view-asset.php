...
<div class="card mb-4 view-container">
    <div class="card-header bg-info text-white d-flex justify-content-between align-items-center">
        ...
        <button type="button" class="btn btn-sm btn-light">Retour</button>
    </div>
    <div class="card-body">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Store AJAX handler URL - direct path to avoid rewrite rules
    const ajaxHandlerUrl = '/modules/planificator/modules/asset-management/ajax-handler.php';
    
    // Handle delete button with AJAX confirmation
    const deleteButton = document.querySelector('button[name="delete_asset"]');
    if (deleteButton) {
        deleteButton.addEventListener('click', function(e) {
            e.preventDefault();
            if (confirm('Êtes-vous sûr de vouloir supprimer cet actif?')) {
                const assetId = this.closest('form').querySelector('input[name="asset_id"]').value;
                
                // Use AJAX to delete the asset
                fetch(`${ajaxHandlerUrl}?action=delete_asset&id=${assetId}`)
                    .then(response => response.json())
                    .then(data => {
                        if (data.success) {
                            // Redirect to the asset list with success message
